...
void Camera::smoothInputs(GLFWwindow* window, glm::vec3* bodyPos[]) {
	//keyboard controls
	if (!focusMode) {
...
void Camera::hardInputs(GLFWwindow* window, glm::vec3* bodyPos[], int numBodies) {
	...
	if (focusMode) {
		...
		if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) { // move backward through object list focus
			focusBody == 0 ? focusBody = numBodies - 1 : focusBody -= 1;
		}
		if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) { // move forward through object list focus
			focusBody == numBodies - 1 ? focusBody = 0 : focusBody += 1;
		}		
